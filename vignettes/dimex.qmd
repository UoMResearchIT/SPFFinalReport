...
# Don't run any steps but print out the banner to show each step that would be run
run_workflow(banner_only = TRUE)

# Run steps 4a and 4b only, with the specified config
# See the [configuration](config.html) vignette for information on how to create a
# non-default config
# TODO: config = ...
run_workflow("4a, 4b", config)
```

## Development

For development work on the `dimex` package, see the
