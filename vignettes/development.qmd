...
This vignette deals with how to use the `spf` package as a developer. See also
the [introduction to spf](spf.html) and [configuration](config.html) vignettes.
...
- You can specify environment variables to control whether to run the
  ...
  `.Renviron` file at the root of the package directory:
    ...
    ````
    SPF_RUN_ALL_HLDT=true  # Set to 'true' or 'false' (or do not specify)
    SPF_RUN_HLDT_1*=true   # Set to 'true' or 'false' (or do not specify)
    ````

    That is, either set `SPF_RUN_ALL_HLDT=true`, or for each test you wish to
    run, set the appropriate `SPF_RUN_HLDT_1*=true`, for example:
    
    ````
    SPF_RUN_HLDT_1A=true
    SPF_RUN_HLDT_1D=true
    ````
    ...
    *not* run.
    ...
    Use `SPF_RUN_ALL_HLDT=true` to run all the high-level data tests,
    irrespective of any `SPF_RUN_HLDT_1*` values. In other words, if
    `SPF_RUN_ALL_HLDT=true` it will override the individual settings. This can
