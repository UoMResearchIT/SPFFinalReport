...
#' Process the Time Use Survey data
#'
#' This function implements a data preparation step to process the Time Use
...
run_data_prep_tus <- function() {
  ...
  uktus15_individual <- uktus15_individual %>%
    ...
    dplyr::mutate(pid_tus = as.numeric(paste(serial, pnum, sep = '')),
                  sex_label = dplyr::case_when(DMSex == 2 ~ 'Female',
                                               DMSex == 1 ~ 'Male'),
                  sex = dplyr::case_when(DMSex == 2 ~ 0,
                                         DMSex == 1 ~ 1),
                  ...
                  pwkstat_label = dplyr::case_when(WorkSta %in% c(-9, -8, -7, -2) ~ "Unknown",
                  ...
                  pwkstat = dplyr::case_when(WorkSta %in% c(-9, -8, -7, -2) ~ 99,
  ...
  uktus15_individual <- uktus15_individual %>%
    ...
    dplyr::mutate(soc2010_label = dplyr::if_else(is.na(soc2010_label), 'Not applicable', soc2010_label),
                  soc2010group_label = dplyr::case_when(soc2010group == -1 ~ "Not applicable",
    ...
    dplyr::mutate(nssec_label = dplyr::if_else(nssec == -1, "Not applicable", nssec_label),
    ...
    dplyr::left_join(readr::read_csv("Data_ref/Raw/Misc/sic2007_classification.csv") %>%
                     ...
                     by = 'sic2') %>%
    # Tidying labels and setting missings as non employed
    dplyr::mutate(sic1 = dplyr::if_else(is.na(sic1), 'ZZZ', sic1),
                  sic1_label = dplyr::if_else(is.na(sic1_label), 'Not applicable', sic1_label),
                  sic2_label = dplyr::if_else(is.na(sic2_label), 'Not applicable', sic2_label),
                  sicgroup_label = dplyr::case_when(sicgroup == 1 ~ "Agriculture, forestry and fishing",
  ...
  uktus15_diary_wide <- uktus15_diary_wide %>%
    ...
    dplyr::mutate(pid_tus = as.numeric(paste(serial, pnum, sep = '')),
                  act_id = 1:dplyr::n(),
                  day_label = dplyr::case_when(DiaryDay_Act == 1 ~ 'Sunday',
                  ...
                  daytype = dplyr::case_when(DiaryDay_Act %in% c(1,7) ~ 1,
                                             DiaryDay_Act %in% 2:6 ~ 2),
                  daytype_label = dplyr::case_when(DiaryDay_Act %in% c(1,7) ~ 'Weekend',
                                                   DiaryDay_Act %in% 2:6 ~ 'Weekday'),
                  season = dplyr::case_when(dmonth %in% c(12, 1, 2) ~ 1,
                                            dmonth %in% c(3:5) ~ 2,
                                            dmonth %in% c(6:8) ~ 3,
                                            dmonth %in% c(9:11) ~ 4),
                  season_label = dplyr::case_when(dmonth %in% c(12, 1, 2) ~ 'Winter',
  ...
  uktus15_diary_wide <- uktus15_diary_wide %>%
    ...
    reshape2::melt(id.vars=c('pid_tus', 'act_id', 'date', 'day', 'day_label', 'daytype', 'daytype_label',
                             'season', 'season_label', 'weights_diary', 'weights_ind'),
                   value.name = 'value',
                   variable.name = 'time') %>%
    # Turning label into numeric
    dplyr::mutate(tmp = dplyr::case_when(substr(time, 1, 4) == 'act1' ~ 'activity',
  ...
  tus_dat <- tus_dat %>%
    ...
    dplyr::filter(nssec != -9) %>%
    # Adding a new flag to know the level of missingness
    # in location for each activity ID
    dplyr::left_join(plyr::ddply(tus_dat,
                                 # Note: Using the .(act_id) syntax causes a
                                 # name conflict with dplyr - so use the formula
                                 # syntax for the 'group by' var instead, i.e.
                                 #   '~act_id' rather than '.(act_id)'
                                 ~act_id,
                                 plyr::summarize,
