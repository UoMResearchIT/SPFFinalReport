...
uktus15_individual <- read_dta("original/Raw/TimeUseSurvey/uktus15_individual.dta")
...
uktus15_individual <- uktus15_individual %>%
  ...
  left_join(read_csv("original/Raw/Misc/soc2010_classification.csv") %>%
...
uktus15_individual <- uktus15_individual %>%
  ...
  left_join(read_csv("original/Raw/Misc/nssec_classification.csv") %>%
...
uktus15_individual <- uktus15_individual %>%
  ...
  left_join(read_csv("original/Raw/Misc/sic2007_classification.csv") %>%
...
uktus15_diary_wide <- read_dta("original/Raw/TimeUseSurvey/uktus15_diary_wide.dta")
...
#save(uktus15_diary_wide, file = "original/Processed/TimeUseSurvey/uktus15_diary_wide_MELTED.RData")
...
uktus15_diary_wide <- uktus15_diary_wide %>% 
  ...
  reshape2::dcast(pid_tus + act_id + date + day + day_label + daytype 
                  + daytype_label + season + season_label + weights_diary 
                  + weights_ind + time ~ tmp,
                  value.var = "value") #%>%

save(uktus15_diary_wide, file = "original/Processed/TimeUseSurvey/uktus15_diary_wide_DCAST.RData")

uktus15_diary_wide <- uktus15_diary_wide %>% 
  # Sorting dataset
  arrange(act_id, time) %>%
  # Merging on time point labels
  left_join(read_csv("original/Raw/TimeUseSurvey/uktus_metadata_timepoints.csv"),
           by = 'time') %>%
  # Merging on activity labels
  left_join(read_csv("original/Raw/TimeUseSurvey/uktus_metadata_activity.csv"),
            by = 'activity')%>%
